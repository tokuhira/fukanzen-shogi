...
**Collision resolution** was designed so that it changes only the *certainty* of an outcome, never its *expected value* — though Rule v0.7 (backed-capture forfeit, below) deliberately reintroduces a material cost for capturing *defended* pieces:
...
- **Backed-capture forfeit (4.8, v0.7 — "討ち死に" / "death in battle"):** Capturing a *defended* piece now carries a cost. The moment the capture completes, the defenders bearing on the capture square (`d`, counted with the just-captured piece removed, so sliders skewered behind it count too) are compared against the capturer's own supporters excluding the piece that just moved (`a`); if `d > a`, the capturing piece is *forfeited* to the opponent's hand and the square is vacated — the captured piece is still gained, but the capturer dies in the exchange. `a ≥ d` resolves as an ordinary capture, unchanged. Kings are exempt (a king's capture is governed by the king-death rules, not this one). This restores the material cost that simultaneous play had quietly removed, and rewards massing enough attackers to break through.
...
- `tui/` — interactive verification desk and **network battle mode** (ratatui + crossterm), over LAN (`tui/src/net.rs`, TCP) or **the cloud** (`tui/src/net_ws.rs`, WebSocket + TLS — joins the same Cloudflare Durable Object room as a browser player).
...
- **Protocol core unification: the TUI joins the cloud** (v0.11.2→v0.12.0, four-stage arc; full account in [`docs/archive/protocol-unification_総括_第一段から第四段.md`](docs/archive/protocol-unification_総括_第一段から第四段.md)) — the TUI's LAN (TCP) and the web's cloud (Durable Object over WebSocket) each had their own hand-rolled wire vocabulary and session-orchestration logic, duplicating the same commit-reveal-ack driving twice. This arc unified both onto a single core:
  - *Stage 1* (`protocol` crate, `PROTOCOL_VERSION` 4→5) — added `WireMessage` (a single `serde`-tagged enum, the canonical wire vocabulary for hello/commit/reveal/ack/reconnect) and `ClientSession` (transport-agnostic session orchestration: handshake, commit-reveal-ack turn driving, reconnect) to `protocol/`. Pure Rust, zero I/O, matching the crate's existing determinism. Reconnect was redefined here: the raw shared secret never touches the wire again — identity is verified by matching the `auth_hash` recorded at the initial handshake, while resume-point lookup still scans kifu position hashes as before.
  - *Stage 2* (`protocol-wasm` thinned, web `?v=` 0.11.12) — `ProtocolSession` (previously a hand-rolled JSON-string state machine duplicating the TCP-side logic) became a thin `wasm_bindgen` wrapper over `ClientSession`. The JS-facing contract and wire bytes are preserved byte-for-byte; only the reconnect path changed, to match the new core-verified identity check.
  - *Stage 3* (v0.11.3, TUI onto native `ClientSession`) — `tui/src/net.rs` shrank to a pure `WireMessage` TCP transport (length-prefixed framing only; the hand-rolled `NetMessage` enum and its separate version-negotiation handshake are gone). `tui/src/online.rs`'s handshake, turn loop, and reconnect logic now drive the same persistent `ClientSession` the web client uses. The LAN wire format moved to PROTOCOL 5, making this TUI build incompatible with pre-v0.11.3 LAN peers — an intentional consequence of unifying the wire vocabulary.
  - *Stage 4* (v0.12.0, cloud participation — the arc's payoff) — a new WebSocket transport (`tui/src/net_ws.rs`, synchronous `tungstenite` + `rustls`, no async runtime) lets the TUI join the very same Cloudflare Durable Object room a browser player would, via a new "cloud battle (room key)" portal option. Side assignment now comes from the DO (`peer_joined`/`room_ready`) rather than a Listen/Connect choice; reconnection rides the DO's `you_reconnected`/`peer_reconnected` framing and reuses the core-level identity check and resume-point search built in stage 3. LAN is untouched and still available side-by-side. A TUI player can now face a browser opponent, or two TUI instances can meet in the same cloud room. **Known limitation (resolved in v0.12.3, see below):** only the browser (Sente) side currently sends the spectate/record-keeper messages, so a TUI-hosted cloud game is not yet live-spectated or archived — carried forward as follow-up work.
- **Fixing a production send-latency bug in TUI cloud mode** (v0.12.1) — real-world post-release testing turned up commits taking tens of seconds to reach the server in cloud mode. Root cause: `tui/src/net_ws.rs` shared a `WebSocket` behind an `Arc<Mutex<...>>` between a reader thread doing tight unlock→relock cycles (bounded by a 200 ms read timeout) and an occasional writer thread — the writer kept losing the lock race, worse the longer the idle period. Fixed by a single-IO-thread redesign: one thread exclusively owns the socket, does a blocking read bounded by the timeout, then drains an `mpsc::Receiver<String>` of pending outgoing messages non-blockingly each loop iteration — no lock, no spin, deterministic worst-case latency bounded by the read timeout. Verified against the real Durable Object (post-idle send delays dropped from tens of seconds to under 1.5s).
- **Terminal-state unification: closing the 500-kumite gap** (v0.12.1→v0.12.2, four-step arc; full account in [`docs/archive/終局判定の単一正本化_総括_StepAからD.md`](docs/archive/終局判定の単一正本化_総括_StepAからD.md)) — `engine::terminate::evaluate` was already the single authority for board-level termination, but the mapping from its result to the archive vocabulary (`ResultKind`/`Outcome`) lived only as an inline match inside `engine-wasm`, and the TUI's `resolve_turn` didn't call `evaluate` at all — it re-implemented king-death/checkmate/sennichite detection by hand and never checked the 500-kumite cap, so **a TUI game would silently keep running past move 500 while a web opponent in the same cloud room correctly ended it, desyncing the two clients**. Resignation's winner logic was separately hand-rolled in three places (TUI online, TUI local, web) as well. This arc unified both:
  - *Step A* (`engine`) — moved the `Terminal → (ResultKind, Outcome)` mapping out of `engine-wasm` into a pure, exhaustively-matched `engine::terminate::terminal_to_result`, byte-for-byte identical to the mapping it replaced. `evaluate` itself unchanged.
  - *Step B* (`protocol`) — added `protocol::game_result(kifu) -> Option<(ResultKind, Outcome)>`, the single window that merges resignation (checked first, since resignation is a protocol-layer declaration, not a board event — `engine::resolve()` already refuses `Action::Resign`) with board termination (delegated to Step A's function).
  - *Step C* (`engine-wasm`, web `?v=` 0.12.0→0.12.1) — `evaluate_terminal` now calls `terminal_to_result` instead of duplicating its match arms. Output verified byte-identical (both by crate tests and by driving the rebuilt Wasm binding directly from Node); web's behavior is unchanged.
  - *Step D* (v0.12.2 — the arc's payoff) — TUI's `resolve_turn` and the online resignation branch now delegate to `protocol::game_result` instead of hand-rolled detection, closing the 500-kumite gap structurally and unifying resignation's winner logic across all three call sites. Verified against the real binary: a synthetically-generated 499-ply non-repeating kifu was loaded via the TUI's existing save/load feature, the 500th move played live over a scripted terminal session, and the game correctly ended in a draw ("引き分け（最長手数・500組手）") instead of continuing.
- **Hardening against a malicious opponent or kifu file** (v0.12.2→v0.12.3) — a security review found that `engine::resolve()` documents "both actions already legal" as the caller's responsibility, but neither the TUI's online turn-resolution path nor its plain-kifu loader actually checked this before calling it: a peer revealing a syntactically-valid-but-illegal move (e.g. moving from an empty square, promoting a king) crashed the process, and a phantom piece-drop silently underflowed a hand count in release builds. Fixed by validating the peer's revealed action (and every loaded kifu ply) against `engine::movegen::legal_actions` before it ever reaches `resolve()`, aborting the connection (or rejecting the file) on failure; `engine::board::Hand::remove`'s bounds check was also hardened to fire in release builds, not just debug. The same class of gap existed in the web client's `handleTurnComplete` (no validation at all) and its archive loader (crash-safe via try/catch, but not prevented) — both were closed the same way. Verified against the real engine crate (each panic reproduced before the fix, then confirmed gone after) and end-to-end: a scripted malicious TCP peer was driven against a live TUI process and the honest side survived with a clean abort message instead of crashing.
- **Cloud spectating for a TUI-hosted game (extension 4b, revised)** (v0.12.3; full account in [`docs/archive/implementation/`](docs/archive/implementation/)) — closes the known limitation left by Stage 4 above. When the TUI is Sente in a cloud room, it now sends the same `spectate_meta`/`spectate_turn`/`spectate_result` messages the web client already sent, so a `/watch` spectator sees the game live regardless of which client is hosting it. `spectate_result` calls `protocol::game_result` directly rather than re-deriving a result-vocabulary mapping by hand (the first draft of this extension, written before the terminal-state unification arc above, would have needed exactly that hand-rolled table — this revision needed none). Gote and LAN are untouched (no spectators exist off cloud); the record-keeper flow (persistent archive, two-witness testimony) is deliberately not part of this — TUI still can't accept a `record_invite`, so a TUI-hosted game still isn't archived, only watchable live. Verified against the real Durable Object: a live TUI process (Sente), a scripted web Gote peer, and a `/watch` spectator page all interacting over the production room — the spectator's board correctly showed both players' moves and, in a second run, the final result of a mutual resignation ("引き分け（投了）"), with the `version` field showing the TUI's own rule/protocol/app tuple.
- **board.js view purification: `render()` becomes a thin shell** (web `?v=` 0.12.3→0.12.5, three-stage arc; full account in [`docs/archive/board.js_view純粋化_総括_View1からView3.md`](docs/archive/board.js_view純粋化_総括_View1からView3.md)) — `render()`'s "state → display value" derivation (labels, buttons, board overlay, cursor interactivity) was still inline in the function itself, even after the earlier board-split arc had extracted everything else into pure modules; a rule change touching terminal messages or phase text meant editing untested code inside a DOM-writing function. This arc finished the job:
  - *View-1* (labels) — `phaseText`/`moveText`/`eventText`/`archiveInfo`/`step`/`total` moved to a pure `labelView(state, gameOverMsg)` in a new `web/view-model.js`, along with the watch/online phase-text helpers (state-argument instead of reading module-global `state`).
  - *View-2* (buttons) — `next`/`prev`/`resign`/`save`/spectate-related button state moved to `buttonView`. Caught and preserved a subtle existing behavior along the way: the "prev" button stays enabled while an input is in progress specifically so a click can cancel it (`goPrev()`'s own logic) — confirmed via the real `goPrev` implementation rather than assumed, and the golden-snapshot test was written to match reality, not intuition.
  - *View-3* (overlay/cursor, the arc's payoff) — the remaining overlay and cursor-interactivity logic moved to `overlay(state)`/`cursorInteractive(state, gameOverMsg)`, composed with the earlier two into a single `viewModel(state, gameOverMsg)`. `render()` now only computes the two wasm-dependent values (`pos` via `parseSfen`, `gameOver` via the memoized `getGameOverMsg`), calls `viewModel` once, and writes the result to the DOM — exactly the target shape the arc's overview doc had sketched at the outset.
  All three stages kept the pure `view-model.js` wasm-free (wasm results are always injected as arguments), so it runs under plain Node/vitest with no build step — 61 new golden-snapshot tests were added, growing the suite from 57 to 118, and each stage was additionally confirmed in a real browser (Playwright) to produce byte-identical DOM output before and after the refactor. `board.js` shrank from 1263 to 1130 lines. Distribution and engine/protocol/tui/server untouched throughout.
- **Rule v0.7 — a backed capture can now backfire ("討ち死に")** (v0.12.3→**v0.13.0**, `RULE_VERSION` 0.6→0.7; full account in `docs/archive/implementation/rule-v0.7-support-forfeit/`) — closes a gap the shift to simultaneous-move play had left open in the ruleset's loss/gain accounting: capturing an opponent's piece used to be free of immediate risk even when the capture square was well-defended, because retaliation was always deferred to a later turn and could be sidestepped. Now, at the instant a capture completes, the defender's attacker count on the capture square (`d`, counted on a board with the just-captured piece removed, so skewered pieces behind it count too) is compared against the capturer's own attacker count excluding the piece that just moved (`a`); if `d > a`, the capturing piece is **forfeited** to the opponent's hand and the square is vacated instead of staying put — the captured piece is still gained normally, only the capturer's piece is lost. `a ≥ d` still resolves as an ordinary capture, unchanged. Kings are exempt from forfeiture (a king's capture is governed by king-death rules, not this one). One rule now serves two of the version-lineage policy's goals at once: forfeiture restores the "captures carry a real cost" guarantee, and being able to mass enough attackers to flip `a ≥ d` rewards active piece coordination — a breakthrough incentive without a separate scoring layer (tried first; a points-based variant remains a fallback branch if this doesn't hold up in play). Landed in three steps: engine core (`engine::resolve::resolve_independent`'s new forfeit branch computes both sides' counts before either is applied, so simultaneous forfeitures on both sides stay order-independent), then a display fast-follow (TUI's `build_resolution_text` prints a forfeit line per side; `engine-wasm`'s `resolve` reports `event:"forfeit"` when a forfeiture occurred, which `web/view-model.js`'s `EVENT_LABEL` renders as "討ち死に" — picked up automatically by live play, replay, and spectating alike, since spectators recompute the same event from the Wasm engine), then this release: `design/不完全将棋_ルール仕様_v0.7.md` became the authoritative rule spec (v0.6 moved to `docs/archive/rule-history/`), and the distribution binary moved from v0.12.3 to v0.13.0 — the version-compatibility policy's first real (rule version, protocol version) tuple bump. Since compatibility is judged by strict tuple equality, every client had to move to v0.13.0 together for online play to keep working.
...
- `tui/` — 対話的検証卓と**ネットワーク対戦モード**（ratatui + crossterm）。LAN（`tui/src/net.rs`、TCP）とクラウド（`tui/src/net_ws.rs`、WebSocket + TLS——ブラウザのプレイヤーと同じ Cloudflare Durable Object の部屋へ入る）の両方に対応。
...
- **通信核の一本化: TUI がクラウドへ参加**（v0.11.2→v0.12.0、四段のアーク。詳細は総括 [`docs/archive/protocol-unification_総括_第一段から第四段.md`](docs/archive/protocol-unification_総括_第一段から第四段.md)）— TUI の LAN（TCP）と web のクラウド（Durable Object 経由 WebSocket）は、それぞれ独自の手組みワイヤ語彙とセッション orchestration を持ち、同じ commit-reveal-ack 駆動を二重に実装していた。本アークで両者を単一の核へ一本化した:
  - **第一段**（`protocol` クレート、`PROTOCOL_VERSION` 4→5）— ワイヤ語彙の正本 `WireMessage`（単一の `serde` タグ付き enum。hello/commit/reveal/ack/reconnect）と、トランスポート非依存のセッション orchestration `ClientSession`（handshake・commit-reveal-ack のターン駆動・再接続）を `protocol/` へ新設。純粋 Rust・I/O ゼロで、クレート既存の決定性を継承。再接続をここで再定義: 生の共有秘密を二度とワイヤに乗せず、初回 handshake で記録した `auth_hash` との一致で本人照合し、再開点探索は従来どおり棋譜局面ハッシュの走査で行う。
  - **第二段**（`protocol-wasm` を痩身、web `?v=` 0.11.12）— 手組み JSON 文字列の状態機械（TCP 側ロジックと二重管理だった）だった `ProtocolSession` を、`ClientSession` の薄い `wasm_bindgen` ラッパへ。JS 向け契約とワイヤバイト列は一字一句保存、変わったのは再接続の照合方式のみ（核による照合へ）。
  - **第三段**（v0.11.3、TUI をネイティブ `ClientSession` へ）— `tui/src/net.rs` は純粋な `WireMessage` の TCP トランスポートへ痩せた（長さプレフィックスの framing のみ。手組みの `NetMessage` enum と独立した版交渉ハンドシェイクは撤去）。`tui/src/online.rs` の handshake・turn loop・再接続は、web クライアントと同じ永続 `ClientSession` で駆動する。LAN のワイヤは PROTOCOL 5 へ移り、v0.11.3 より前の TUI とは LAN 上で非互換になる（ワイヤ語彙統一の意図した帰結）。
  - **第四段**（v0.12.0、クラウド参加——アークの結実）— 新設の WebSocket トランスポート（`tui/src/net_ws.rs`、同期 `tungstenite`＋`rustls`、非同期ランタイムなし）により、TUI がブラウザのプレイヤーと全く同じ Cloudflare Durable Object の部屋へ、新設の「通信対戦（クラウド・部屋キー）」ポータル選択肢から入れるようになった。陣営はポータルの Listen/Connect 選択でなく DO（`peer_joined`/`room_ready`）が告げる。再接続は DO の `you_reconnected`/`peer_reconnected` の枠組みに乗り、第三段で確立した核レベルの本人照合・再開点探索をそのまま再利用する。LAN は無変更で併存。TUI のプレイヤーがブラウザの相手と対戦する、あるいは二つの TUI が同じクラウドの部屋で落ち合うことができるようになった。**既知の限界（v0.12.3 で解消・後述）:** 生観戦・記録係向けメッセージの送出はいまもブラウザ側（先手）クライアントのみが担うため、TUI がホストするクラウド対局は現時点で観戦・アーカイブされない——今後の課題として持ち越し。
- **クラウドモードの送信遅延バグを修正**（v0.12.1）— 本番リリース後の実地確認で、クラウドモードでの着手確定がサーバへ届くまで数十秒かかることが発覚。原因は `tui/src/net_ws.rs` が `Arc<Mutex<WebSocket>>` を、読み取りタイムアウト（200ms）ごとに unlock→即 relock を繰り返す reader スレッドと、送信を担う別スレッドとで共有していたこと——送信側がロックを取り損ね続ける実質的なロック飢餓が起き、アイドル時間が長いほど悪化していた。単一 IO スレッド設計へ作り替えて修正: 一つのスレッドだけがソケットを排他所有し、タイムアウト付きブロッキング読み取り→送信待ちキュー（`mpsc::Receiver<String>`）の非ブロッキング排出、を交互に繰り返す——ロックもスピンも無く、最悪送信遅延は読み取りタイムアウトの周期に決定的に収まる。実 Durable Object で検証（アイドル後の送信遅延が数十秒から1.5秒未満へ）。
- **終局判定の単一正本化: 500組手の穴を塞ぐ**（v0.12.1→v0.12.2、四段のアーク。詳細は総括 [`docs/archive/終局判定の単一正本化_総括_StepAからD.md`](docs/archive/終局判定の単一正本化_総括_StepAからD.md)）— `engine::terminate::evaluate` は既に盤面終局判定の単一の権威だったが、その結果をアーカイブ語彙（`ResultKind`/`Outcome`）へ写すマッピングは `engine-wasm` 内のインライン match としてのみ存在し、TUI の `resolve_turn` は `evaluate` を一切呼ばず玉の死・詰み・千日手判定を手組みで再実装していて、**最長手数500組手の上限を一度もチェックしていなかった**——同じクラウドの部屋にいる web の相手が正しく終局する一方で、TUI だけが500組手を超えて進行を続け、両者が desync する実害があった。投了の勝敗判定も TUI online・TUI local・web の三箇所でそれぞれ手組みされ、乖離していた。本アークで両者を一本化した:
  - **StepA**（`engine`）— `Terminal → (ResultKind, Outcome)` のマッピングを `engine-wasm` から取り出し、網羅列挙の純粋関数 `engine::terminate::terminal_to_result` として engine 本体へ移設（移設元と一字一句一致）。`evaluate` 自体は無変更。
  - **StepB**（`protocol`）— 投了（盤の出来事でなくプロトコル層の宣言——`engine::resolve()` は既に `Action::Resign` を拒否している）と盤面終局を合流させる単一窓口 `protocol::game_result(kifu) -> Option<(ResultKind, Outcome)>` を新設。投了を先に判定し、そうでなければ StepA の関数へ委譲。
  - **StepC**（`engine-wasm`、web `?v=` 0.12.0→0.12.1）— `evaluate_terminal` 内のインライン match を `terminal_to_result` の呼び出しへ置換。出力がバイト単位で不変であることを crate テストに加え、再ビルドした wasm バインディングを Node から直接叩いて確認。web の挙動は不変。
  - **StepD**（v0.12.2——アークの結実）— TUI の `resolve_turn` と online の投了枝を `protocol::game_result` への委譲へ書き換え、**500組手の穴を構造的に塞ぎ**、投了の勝敗判定を三箇所の手組みから単一正本へ寄せた。実機で検証: 反復のない499手ぶんの棋譜を Node スクリプトで生成し、TUI 既存の保存・読込機能で読み込んでから500手目を実際に指すスクリプト化された端末セッションを実行し、対局が続行せず正しく「引き分け（最長手数・500組手）」で終局することを確認した。
- **悪意ある対戦相手・棋譜に対する耐性強化**（v0.12.2→v0.12.3）— セキュリティレビューで発覚: `engine::resolve()` は「両着手が既に合法」を呼び出し側の責任として文書化しているが、TUI のオンライン対戦経路にも棋譜読込経路にもこの検証が存在しなかった。相手が構文上は正しいが非合法な着手（例: 駒のないマスからの移動、玉の成り宣言）を reveal すると resolve() がパニックしてプロセスが落ち、持っていない駒の phantom drop は release ビルドで持ち駒カウントをサイレントにアンダーフローさせていた。相手の reveal（および読み込んだ棋譜の全 ply）を `engine::movegen::legal_actions` で検証してから `resolve()` へ渡すよう修正し、失敗時は接続をアボート（または棋譜読込を拒否）するようにした。`engine::board::Hand::remove` の境界チェックも release ビルドで発火するよう強化。同じ穴は web の `handleTurnComplete`（無検証）とアーカイブ読込（try/catch でクラッシュは防げていたが未然防止ではなかった）にもあり、同様に塞いだ。実 engine crate で各パニックを修正前に再現・修正後に消えたことを確認し、実機でも: 悪意ある TCP peer を模したスクリプトを実際の TUI プロセスへ向けて駆動し、honest 側がクラッシュせず明確なアボートメッセージで生き残ることを確認した。
- **TUI 先手のクラウド対局の観戦配信（延長4b・改訂版）**（v0.12.3。詳細は `docs/archive/implementation/` 配下）— 上記第四段が残した既知の限界を塞ぐ。TUI がクラウドの部屋で先手のとき、web クライアントが既に送っていたのと同じ `spectate_meta`/`spectate_turn`/`spectate_result` を送出するようになり、どちらのクライアントがホストしていても `/watch` の観戦者が生で見られるようになった。`spectate_result` は結果語彙のマッピングを手組みせず `protocol::game_result` を直接呼ぶ（この延長の初版は終局判定の単一正本化アークより前に書かれておりまさにその手組み表を必要としていたが、この改訂版では不要になった）。後手・LAN は無変更（クラウド以外に観戦者はいない）。記録係フロー（永続書庫・二証人証言）は意図的に含まない——TUI はいまも `record_invite` を受諾できないため、TUI がホストする対局は生観戦はできてもアーカイブはされない。実 Durable Object で検証: 実際の TUI プロセス（先手）・web の後手役を演じるスクリプト・`/watch` の観戦ページを本番の部屋で相互接続し、観戦者の盤面が両者の着手を正しく反映すること、別の実行では両者投了の最終結果（「引き分け（投了）」）が正しく表示され、`version` 欄に TUI 自身のルール／プロトコル／アプリ版タプルが出ることを確認した。
- **board.js view 純粋化: render() が薄い殻になる**（web `?v=` 0.12.3→0.12.5、三段のアーク。詳細は総括 [`docs/archive/board.js_view純粋化_総括_View1からView3.md`](docs/archive/board.js_view純粋化_総括_View1からView3.md)）— board.js 分割アークが他のすべてを純粋モジュールへ抜いた後も、`render()` 自身の「state → 表示値」の導出（ラベル・ボタン・盤面 overlay・cursor の操作可能判定）はまだ関数の中にベタ書きされたままだった——終局メッセージや phaseText に触るルール変更は、テストの無い DOM 書き込み関数の中身を直接編集する形になっていた。本アークでこれを締めた:
  - **View-1（ラベル）** — `phaseText`/`moveText`/`eventText`/`archiveInfo`/`step`/`total` を、新設 `web/view-model.js` の純粋関数 `labelView(state, gameOverMsg)` へ移した。観戦・オンラインの phaseText ヘルパも module global の `state` 参照から引数化。
  - **View-2（ボタン）** — `next`/`prev`/`resign`/`save`/観戦関連のボタン状態を `buttonView` へ。途中で一見直感に反する既存挙動（入力途中でも「前へ」ボタンが無効化されず、押すと入力をキャンセルする——`goPrev()` 自身のロジック）を発見し、思い込みではなく実際の `goPrev` の実装を確認した上で、golden snapshot テストを直感でなく現実の挙動に合わせて書いた。
  - **View-3（overlay/cursor・アークの結実）** — 残る overlay と cursor 操作可能判定を `overlay(state)`/`cursorInteractive(state, gameOverMsg)` へ抜き、先の二つと合成した単一の `viewModel(state, gameOverMsg)` を置いた。`render()` はいまや wasm 依存の値二つ（`parseSfen` 経由の `pos`、メモ化済み `getGameOverMsg` 経由の `gameOver`）だけを計算し、`viewModel` を一度呼び、結果を DOM へ書くだけになった——アーク概観が最初に描いた目標構造そのもの。
  三段とも純粋な `view-model.js` は wasm フリーのまま保った（wasm の結果は常に引数として注入する）ため、ビルド不要の素の Node／vitest で走る——golden snapshot テストを 61 本追加し、テスト総数は 57→118 件に増えた。各段はさらに実ブラウザ（Playwright）でリファクタ前後の DOM 出力がバイト単位で一致することを確認した。`board.js` は 1263→1130 行に縮んだ。配布版・engine/protocol/tui/server は終始無変更。
- **ルール v0.7 — 支え付き取得の討ち死に**（v0.12.3→**v0.13.0**、`RULE_VERSION` 0.6→0.7。詳細は `docs/archive/implementation/rule-v0.7-support-forfeit/`）— 同時着手化がルールの損得勘定に開けていた穴を塞いだ。支えのある駒を取っても、取り返しは常に次ターンへ繰り延べられ回避可能だったため、取得そのものには即時のリスクが無かった。これからは、取得が完了した瞬間に、取られたマスへの守り側の利き数（**d**——取られた駒を盤から除いて数えるので、串刺しの後詰めも数に入る）と、いま動いた駒を除いた攻め側自身の利き数（**a**）を比較し、**d ＞ a** なら取りに行った駒を**相手の駒台へ没収**してマスを空ける（取られた駒はそのまま自分の駒台へ——取得自体は成立、没収されるのは取りに行った駒だけ）。**a ≧ d は従来どおり通常取得**。玉は没収の対象外（玉の取得は本規則でなく玉の死のルールが担う）。一つの規則がルール版の系譜方針の二目標を同時に担う——没収が「取得には代償が伴う」という保証を回復し、利きを足して a≧d を作れることが積極的な駒運用への報酬（点数制を別途設けずに膠着打開を促す。うまく機能しなければ点数制の分家を後日試す）。着地は三段: engine 核（`engine::resolve::resolve_independent` の没収分岐は両側の利き数を先に数えてから適用するため、双方同時の没収でも順序に依存しない）→ 表示 fast-follow（TUI の `build_resolution_text` が没収行を出力、`engine-wasm` の `resolve` が没収時に `event:"forfeit"` を返し `web/view-model.js` の `EVENT_LABEL` が「討ち死に」と表示——観戦者も wasm で同じ event を再計算するため、生観戦・再生いずれにも自動で波及）→ 本リリース（`design/不完全将棋_ルール仕様_v0.7.md` を正本化し v0.6 を `docs/archive/rule-history/` へ、配布バイナリを v0.12.3→v0.13.0 へ——バージョン互換性方針の初めての実タプル bump）。対戦互換性判定は厳密一致のため、全クライアントが v0.13.0 へ揃って初めてオンライン対戦が成立する。
...
- [Protocol core unification — summary of stages 1–4](docs/archive/protocol-unification_総括_第一段から第四段.md) — v0.11.2→v0.12.0. Unifies the TUI's LAN wire vocabulary and the web's DO/WebSocket session logic into a shared `protocol` core (`WireMessage`, `ClientSession`), then lets the TUI join the same cloud room a browser player would. Individual stage documents live under `docs/archive/implementation/protocol-unification/`.
- [Terminal-state unification — summary of Steps A–D](docs/archive/終局判定の単一正本化_総括_StepAからD.md) — v0.12.1→v0.12.2. Unifies board-termination mapping (`engine::terminate::terminal_to_result`) and resignation-inclusive result determination (`protocol::game_result`) into a single source of truth shared by TUI and web, structurally closing a bug where the TUI never enforced the 500-kumite maximum-length draw. Individual step documents live under `docs/archive/implementation/terminal-unification/`.
- [Extension 4b (revised) — TUI Sente cloud spectate broadcast](docs/archive/implementation/不完全将棋_実装指示書_延長4b改訂_TUI先手の観戦配信_game_result直呼び.md) — v0.12.3. Closes the spectate gap left by the protocol core unification's Stage 4: the TUI now sends `spectate_meta`/`spectate_turn`/`spectate_result` when it hosts a cloud game as Sente, calling `protocol::game_result` directly for the result (no hand-rolled result-vocabulary table, unlike the extension's first draft).
- [board.js view purification — summary of View-1–3](docs/archive/board.js_view純粋化_総括_View1からView3.md) — web `?v=` 0.12.3→0.12.5. Extracts `render()`'s remaining "state → display value" derivation (labels, buttons, overlay, cursor) into pure, wasm-free functions in `web/view-model.js`, composed into a single `viewModel(state, gameOverMsg)`. Individual stage documents live under `docs/archive/implementation/board-js-view-purification/`.
...
# Run the TUI — shows the portal menu (select single-player, LAN online, or cloud mode)
cargo run --bin fukanzen-shogi-tui

# Online battle — Sente side (direct launch over LAN, bypasses portal)
cargo run --bin fukanzen-shogi-tui -- --listen 8765 --secret mypass

# Online battle — Gote side (direct launch over LAN, bypasses portal)
...
**Portal menu** — the no-argument launch opens a menu with four choices: single-player verification desk, LAN battle as Sente (Listen), LAN battle as Gote (Connect), or **cloud battle (room key)** — join the same Cloudflare Durable Object room a browser player would, no side selection needed (the room assigns it). The LAN form asks for the port or address and a shared password; the cloud form asks for a room key and a shared password. After the first game the previous values are pre-filled. When the game ends you return to the portal automatically to start a new one without restarting the binary.

**Direct launch** — passing `--listen` / `--connect` + `--secret` skips the portal and starts a LAN online game immediately, behaving as in earlier releases. (Cloud mode is portal-only; there is no CLI-flag bypass for it.)

In online mode (LAN or cloud) the commit-reveal-ack exchange is fully automatic: the commit is sent the moment you confirm a move, the reveal is sent when both commits arrive, and the ack is sent once the peer's reveal passes verification. The turn resolves on both screens simultaneously. On disconnect, reconnection retries in the background (LAN: the Connect side, up to 60 seconds; cloud: rejoining the same room, riding the Durable Object's reconnect framing); a success banner is displayed and any rolled-back move is flagged for re-entry.
