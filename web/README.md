...
- **Click a piece** to see legal moves as subtle ink dots on the board (v0.7 rules enforced by engine — draw is a first-class result: definite mate, king's death, sennichite, and the 500-kumite max length all end the game and are recorded correctly, not just mate/king's death/resignation as before. **v0.7** adds a risk to capturing a well-defended piece: if the defender's attacker count on the capture square exceeds the capturer's own, the capturing piece is forfeited to the opponent's hand instead of staying on the board — shown as a "討ち死に" event cue).
