...
- **やらないもの（実装者の担当外）**:
  - **§7 のデプロイ一式**——本番鍵の生成・`wrangler secret put`・`wrangler deploy`・本番確認。**実装者は触らない。** 本番を触る操作なので、コードとテストが着地したあと、**別途 Opus と一緒に人が実行する**（§7 冒頭）。実装者の仕事は §6.3 の受け入れ基準までで完了。
- **作らないもの（絶対に触らない）**:
...
- **ここまでで実装者の仕事は完了**。§7 のデプロイへは進まない（担当外・§0）。コミットして手を止めること。

---

## 7. デプロイ（このアーク初）

> **実行者: 人（Opus と一緒に）。実装者（Sonnet）はここを実行しない。** 本番の鍵と稼働中の Worker を触る操作で、失敗の跡が本番に残る。コードとテストが着地して §6.3 を満たしたら、そこで一旦手を止めること。以下は着地後に人が辿る手順書。
...
- **本番デプロイあり**（§7）——ただし**実装とは別の局面**として、人が実行する。デプロイが済んだ時点で初めて「押された封蝋を第三者が検証できる」状態になる。
