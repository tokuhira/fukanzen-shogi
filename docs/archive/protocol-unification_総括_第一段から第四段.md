...
- **TUI が先手のクラウド対局は、まだ生観戦・アーカイブされない**——`spectate_meta`/`spectate_turn`/`spectate_result` の送出と記録係の招待/受諾フローは、この段では意図的に作らなかった（記録係アークと結合すると重くなるため・過ぎたるは及ばざる）。対局自体は成立する（game channel は DO が relay するので web 側の観戦・記録には影響しない——影響が出るのは TUI 自身が先手のときだけ）。バックログ §A・§C の記録係の道と合流する形で別畝として拾う。
